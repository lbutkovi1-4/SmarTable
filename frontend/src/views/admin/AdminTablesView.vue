...
function save() {
  if (editingId.value) {
    const t = tables.value.find(t => t.id === editingId.value)
    Object.assign(t, form.value)
  } else {
    tables.value.push({ id: Date.now(), ...form.value })
  }
  resetForm()
}

function remove(t) {
  if (confirm(`Obrisati stol "${t.name}"?`)) {
    tables.value = tables.value.filter(s => s.id !== t.id)
  }
}
</script>
...
.delete-btn:hover { background: #8c3324; }
</style>